...
La Weekly Tech Meeting est une réunion hebdomadaire permettant un partage de l’avancement de chaque projet en cours. Cela augmente la cohésions des équipes mais aussi de présenter les évolutions des outils internes qui peuvent potentiellement être utilisé par tous.
...
\href{Le site principal de MicroEJ}{https://www.microej.com/} sert de vitrines pour l'entreprise et ces produits. Il contient aussi les informations de contacts clients.

\subsection{Site développeur}

\href{Ce site}{https://developer.microej.com/} contient de nombreux tutoriels allant de la mise en place d'un environnement de développement pour la VEE MicroEJ, à la création de ces applications et jusqu'à l'analyse des performances et l'optimisation du programme.

\subsection{Docs public}

\href{La documentation public}{https://docs.microej.com/en/latest/} contient les informations nécessaires pour le développement d’application sur l’IDE microEJ SDK.
...
Ce Github public contient des exemples d’applications sur la MicroEJ VEE. Il contient aussi la documentation développer ce qui permet de la modifier simplement dès que l'on détecte un problème pendant son utilisation.
...
J'ai créer des icônes permettant de représenté la météo 
...
J'ai ensuite continué de faire retoucher ce projet en parallèle du dependency discoverer. Cela a compris l'ajout de variations de la couleur de fond de l'horloge en fonction de la position du soleil afin de rendre compte l'aube et du crépuscule. J'ai aussi rédigé un README qui présentait les fonctionnalités de l'application et les dépendances sur lesquelles elle repose.
...
Le fonctionnement du dependency discoverer repose sur l'analyse des fichiers class contenant le programme compilé. 
...
\textit{Pour cela on met en place un visitable et un visiteur. Le visitable contienne la structures de données et possèdent une méthode prenant en paramètre un visiteur; durant l’exécution de cette méthode, le visitable va itéré à travers toutes la structure de données et appelé, pour chaque donnée une méthode du visiteur permettant son traitement.}

\textit{java permet l'override, une classe fille héritant d'un classe mère peut redéfinir le contenu d'une méthode. Une classe fille d'un visiteur peut donc être passé en paramètre du visitable correspondant et appliqué un traitement des données personnalisé.}


\bigskip

Pour l'analyse il faut lister les méthodes des Classnodes puis créer une classe qui hérite du visiteur de méthodes standard ASM pour effectuer une visite dans ces noeuds de méthode (seul le code référencé depuis des méthodes sera vraiment utilisé et nécessite une analyse). On peut ensuite réécrire le contenu de ces fonctions pour appeler nos propres fonctions. Dans mon cas j'ai appelé les méthodes existantes permettant de rajouter des dépendences.   
...
Originellement, l'utilisation du dependency discoverer nécessitait de charger le code source dans une variation de l'IDE Eclipse et de l’exécuter dans cet IDE. Cette solution n'est pas pratique, en particulier pour un client externe à l'entreprise. Une \hyperref{CLI}{CLI} est bien plus adaptée.
...
Le mécanisme utilisé est un système d'annotations @NonNull et @Nullable) à placer dans le code permettant à l'IDE de déterminer lorsqu'un accès invalide peut survenir. On renseigne le fichier package-info.java dans chaque paquets java pour mettre @NonNull comme annotation par default :
\begin{lstlisting}
@org.eclipse.jdt.annotation.NonNullByDefault
package packagename;
\end{lstlisting}
on peut ensuite rajouter l'annotation @Nullable quand on à besoin qu'une variable puisse passer dan l'état nulle, l'IDE va ensuite automatiquement prévenir des accès à cette variable avant instanciation.
...
Un exemple est la méthode get de la classe HashMap. Cette classe fait partie des librairie java, elle ne peut donc pas être modifiée, les annotations doivent donc êtres externes. Cette méthode nécessite des annotations car par default NonNull est utilisé pour des librairie externes. Or cette méthode peut renvoyer null et ce même si toute les autres valeurs sont non null : La hashmap est similaire à un dictionnaire, même si le dictionannaire ne contient que des mots et définitions valides mais que le mot que l'on cherche n'y est pas présent on ne récupérera aucune définition.

Pour cette fonction on passe de l’entête :
\begin{lstlisting}
...
J'ai utiliser la Powershell Windows pour vérifier le bon fonctionnement de l’exécutable généré :
\begin{center}
...
J’ai utilisé une image Ubuntu, la distribution Linux la plus complète actuellement disponible.Une fois WSL mis en place j’ai pu générer les variables sécurisées sur mon projet d'expérimentation. Après avoir vérifié la page release sur Github les fichier uploadé sont bien présents.

\begin{center}
  \includegraphics[width=\textwidth]{ressources/images/github.png}
\end{center}

J'ai enfin créer une copie du projet du dependency discoverer sur un Github privé pour adapter le build.
...
Les méthodes sont aussi similaires, pour deux raisons :
-Les contraintes de développement informatique sont similaires quelque soit le domaine (web, appli ...)
-De très bon outils open source et maintenus par la communauté ont émergé et se sont imposés dans le monde.
Je pondérerais avec des contre-exemple comme les chinois dont le niveau d’anglais est souvent trop bas pour que les échanges fluides. Ou les japonais qui communique souvent sur des sites inconnus hors du japons. 
D’un point de vu générale, l’environnement mondial autour de l’IT est pour moins le meilleurs exemple d’une collaboration international fonctionnel.

Dans d’autres cas, deux salles deux ambiances\\
Mon expérience dans l’informatique est très différente de ce qui peut se passé lorsqu’on veut étudier ou travailler dans un autre pays, dans ce cas connaître la langue et la culture devient parfait obligatoire. Pour mon semestre à l’étranger je comptais aller au Japon, la pandémie m’en a empêché mais j’avais cependant suivit toute les procédures d’inscription et même cherché un logement. Le japonais est un langage extrêmement éloigné des langues européennes , les symboles , la structure sont différente. L’anglais est donc très dur à apprendre pour les japonais et inversement apprendre le japonais est très compliqué pour un européen.  
Mon contact sur place était un enseignant-chercheur japonais, son niveau d’anglais était donc suffisant pour que nous puissions communiquer, il m’a aidé pour mes démarches car une partie devait être réalisé sur place. J’ai cependant rencontrer de nombreuses difficultés car la plupart des ressources écrites étaient rédigées exclusivement en japonais. J’ai d’ailleurs dû traduire la liste de cours que j’avais choisit de suivre.Le formulaire de demande de visa était lui aussi en japonais, heureusement mon contact avait traduit les parties les plus compliquées.Enfin ma recherche de logement s'est faites complètement en japonais.

Étudier au japon est donc assez difficile mais pour ce qui est du travail et de l'objectif de s'y installer.
Les codes du monde du travail Japonais sont très stricts, le respect de supérieur est absolue, il faut d'ailleurs utilisé une variation du honorifique language que même les natifs japonais peine à maitriser .
Contrairement à la France très peu d'étrangers vivent au Japon. Toutes les personnes vivant là-bas rapporte que même après des dizains d’années et une maîtrise de la langue et de la culture ils restaient des étranger et subissait une certaine exclusion.

Vivre en France peut d'ailleurs aussi être compliqué pour des étrangers. Durant m'a formation j'ai pu rencontrer différents étudiants étrangers qui on tous rencontré des problèmes. L'un était réfugié Syrien à du reprendre des études d'ingénieurs car son diplôme syrien n'était pas reconnu. Il à ensuite du apprendre le français seul et suivre le cursus sans aménagement.  
Les avoir rencontré m'a permit de ...

Conclusion : S’intégrer dans d'autres pays peut être très compliqué et il est nécessaire de se renseigné avant d'aller travailler à l'étranger. Cela comprend l'apprentissage de la langue mais aussi la culture. Cela peut décourager mais mon expérience m'a montré que partager les cultures permet une meilleurs ouverture d'esprit et de partager des connaissances et compétences ainsi que de mettre au point des projets communautaires à l'influence mondiale.
...
Ce stage m’a quand à lui apporté ce qui me manquait pour compléter ma formation : un cadre et des méthodes pour appliquer mes compétences avec une rigueur suffisante pour assurer une utilisation par ‘n’importe qui’ et une maintenance par d’autre développeurs sans risqué de casser le programme.
