...
MicroEJ Corp est une entreprise dont le produit principal est l'environnement d’exécution virtuel MicroEJ VEE qui permet l’exécution d'applications java sur des cartes électroniques embarquées. L'entreprise assure l'évolution et la maintenance de ce programme ainsi que de différents outils aidant le développement sur cet environnement.

L'un de ces outils est le Dependency Discoverer qui permet de déterminer les possibles dépendances manquantes pour porter un programme java sur la plateforme MicroEJ VEE. Mon travail durant ce stage consistait à la refonte de cet outil afin de le rendre compatible avec les versions les plus récentes de java et de le rendre plus facilement utilisable, en particulier en rajoutant une interface en lignes de commandes. 

Un autre aspect à été l'ouverture à l'open source du code. Étant le premier outils de l'entreprise dans ce cas mon sujet à été utilisé pour explorer 

Ce travail dû être réalisé en respectant le workflow de l'entreprise.

Ce stage m'a permis de développer mes compétences en programmation mais aussi d'exercé ma capacité de recherche et mon esprit critique. J'ai appris à utiliser de nombreux outils permettant la gestion de projets informatiques et ai acquis une compréhension approfondie du langage java.  

\vspace*{\fill}

5 mots clefs : Internet of things, recherches.
...
Vivre en France peut d'ailleurs aussi être compliqué pour des étrangers. Durant m'a formation j'ai pu rencontrer différents étudiants étrangers qui on tous rencontré des problèmes. L'un était réfugié Syrien et à du reprendre des études d'ingénieurs car son diplôme syrien n'était pas reconnu en France. Il a dû apprendre le français seul et suivre le cursus sans aménagements.
Un autre n'avais pas encore effectué la démarche de naturalisation Française (étant d'origine Mongol il perdrait sa nationalité Mongole il serait donc beaucoup plus compliqué d'aller voir de la famille) et devait par conséquent renouveler un visa tout les ans. Cela lui imposait aussi un minimum d'argent d'avance, il a donc du travailler une partie de ces études pour respecté cette contrainte.
...
Pour ces différentes tâches j’ai appris à utiliser de nombreux outils d’assistance aux développement informatique mais aussi à coder différemment en prenant en compte l’interprétation du code par l'environnement virtuel java et la lisibilité pour d'autres développeurs.
